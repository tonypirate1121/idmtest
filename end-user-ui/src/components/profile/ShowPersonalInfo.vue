...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            loadUserdata () {
                let managedUserdata = this.getRequestService({
                    headers: {
                        'X-OpenIDM-NoSession': false,
                        'X-OpenIDM-Password': 'Password1',
                        'X-OpenIDM-Username': this.$root.userStore.state.userName
                    }
                });
                this.testapicall = managedUserdata.get('/user?_queryId=query-all');
            },
            loadwidgetData () {
